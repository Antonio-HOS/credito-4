...
int maiorEmenor(float a[],int tam)
{
    ...
    printf(C_RED "\nos tres menores0 numero do array sao:\n" C_GRAY);
    for (int r = 0; r < 3; r++)
    {
        printf(" %.2f\n", a[r]);
    }
...
int informacoes(float a[],int tam)
{
    ...
    for (int i = 0; i < tam; i++)
    {
        if (auxanterior != a[i])
        {
            ...
            printf("\no numero %.3f se repete %i vez(es) e ocupa o index %i", aux, quantidadedevezes, i);
            quantidadedevezes = 0;
            auxanterior = aux;
        }
    }
...
int main()
{
    ...
    int numeroauxflaot = numero * 100;
    float auxnumberforarr = 0.0;

    int inde = 0;
    for (int i = 0; i < numero; i++)
    {
        auxnumberforarr = rand() % numeroauxflaot;
        arr[i] = auxnumberforarr / 100.0;
    };
    ...
    //     printf("\n\n\n deseja ver como fica as informacoes no novo? [1]sim [0]nao");
    //     scanf("%i", &pergunta);
    //     if (pergunta == 1)
    //     {
    //          maiorEmenor();
